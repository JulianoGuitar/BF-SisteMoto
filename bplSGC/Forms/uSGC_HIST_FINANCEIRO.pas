...
uses
  ...
  Buttons, bfEdit, bfdbEdit, DBGrids, bfdbMemo, RelVisualBlue, dbxpress;

type
  TfmSGC_HIST_FINANCEIRO = class(TForm)
    cds: TClientDataSet;
...
var
  fmSGC_HIST_FINANCEIRO: TfmSGC_HIST_FINANCEIRO;
const
  sCodProg : string = 'SGC010';
...
function SGC010(sOpUsuario, sOpLocal, sOpFiltros, sOpDBSel: pchar; cOpBlueField, cOpNormal : TColor; iEmpresa, iOp: integer; bOpModal: boolean = false): pchar;
begin
  Result := '';
  with TfmSGC_HIST_FINANCEIRO.Create(Application) do
  begin;
    bModal := bOpModal;
...
procedure TfmSGC_HIST_FINANCEIRO.btTransportaClick(Sender: TObject);
begin
  if bModal then
  begin
...
procedure TfmSGC_HIST_FINANCEIRO.FormCreate(Sender: TObject);
begin
  bDBAberto := false;
...
procedure TfmSGC_HIST_FINANCEIRO.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  ...
  if not bModal then
  begin
    fmSGC_HIST_FINANCEIRO := nil;
    Action := caFree;
  end;
end;

procedure TfmSGC_HIST_FINANCEIRO.HabilitaControles;
begin
...
procedure TfmSGC_HIST_FINANCEIRO.btConsultaClick(Sender: TObject);
begin
  if not bDBAberto
  then
  begin
    dtmCon := TdtmConexao.Create(fmSGC_HIST_FINANCEIRO);
  ...
  begin
    ...
    SQL.Clear;
    SQL.Add(' select * from SGC_HIST_FINANCEIRO ');
    SQL.Add(' where ID > 0 ');
...
procedure TfmSGC_HIST_FINANCEIRO.cdsAfterPost(DataSet: TDataSet);
var
  td : TTransactionDesc;
begin
...
procedure TfmSGC_HIST_FINANCEIRO.cdsBeforePost(DataSet: TDataSet);
begin
...
procedure TfmSGC_HIST_FINANCEIRO.cdsIDGetText(Sender: TField; var Text: string;
  DisplayText: Boolean);
begin
...
procedure TfmSGC_HIST_FINANCEIRO.cdsAfterEdit(DataSet: TDataSet);
begin
...
procedure TfmSGC_HIST_FINANCEIRO.cdsAfterInsert(DataSet: TDataSet);
begin
  cdsUSU_INCLUI.AsString := sUsuario;
  HabilitaControles;
end;

procedure TfmSGC_HIST_FINANCEIRO.dspUpdateError(Sender: TObject;
...
procedure TfmSGC_HIST_FINANCEIRO.BeforeUpdateRecord(Sender: TObject; SourceDS: TDataSet;
...
procedure TfmSGC_HIST_FINANCEIRO.dbgConsultaDblClick(Sender: TObject);
begin
  if bModal
  and not cds.IsEmpty
...
procedure TfmSGC_HIST_FINANCEIRO.cdsNewRecord(DataSet: TDataSet);
begin
  cdsID.ReadOnly := false;
  HabilitaControles;
end;

procedure TfmSGC_HIST_FINANCEIRO.cdsAfterCancel(DataSet: TDataSet);
begin
  cdsID.ReadOnly := true;
  HabilitaControles;
end;

procedure TfmSGC_HIST_FINANCEIRO.cdsAfterDelete(DataSet: TDataSet);
var
  td : TTransactionDesc;
begin
  td.TransactionID := Cardinal(FormatDateTime('ddmmyyyy',date)+FormatDateTime('hhnnss',time));
...
procedure TfmSGC_HIST_FINANCEIRO.btSalvaClick(Sender: TObject);
begin
  if Pode_Salvar
  then cds.Post;
end;

procedure TfmSGC_HIST_FINANCEIRO.btImprimeClick(Sender: TObject);
begin
...
procedure TfmSGC_HIST_FINANCEIRO.btIncluiClick(Sender: TObject);
begin
...
procedure TfmSGC_HIST_FINANCEIRO.btDesisteClick(Sender: TObject);
begin
...
procedure TfmSGC_HIST_FINANCEIRO.btExcluiClick(Sender: TObject);
begin
...
procedure TfmSGC_HIST_FINANCEIRO.btSaiClick(Sender: TObject);
begin
  Close;
end;

procedure TfmSGC_HIST_FINANCEIRO.tsConsultaShow(Sender: TObject);
begin
  if cds.State in [dsInsert, dsEdit]
  then cds.Cancel;
...
procedure TfmSGC_HIST_FINANCEIRO.edtConsultaChange(Sender: TObject);
begin
if cds.Active then cds.Close;

end;

procedure TfmSGC_HIST_FINANCEIRO.edtConsultaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmSGC_HIST_FINANCEIRO.edtConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = chr(vk_return) then btConsultaClick(self);
end;

procedure TfmSGC_HIST_FINANCEIRO.dbgConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfmSGC_HIST_FINANCEIRO.
...
procedure TfmSGC_HIST_FINANCEIRO.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfmSGC_HIST_FINANCEIRO.Direitos;
var
  s : string;
begin
  bCad := false;
...
function TfmSGC_HIST_FINANCEIRO.Pode_Salvar: boolean;
begin
  result := false;
