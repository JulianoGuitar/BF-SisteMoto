...
var
  fmSGC_BANCOS: TfmSGC_BANCOS;
const
  sCodProg : string = 'SGC004';
...
function SGC004(sOpUsuario, sOpLocal, sOpFiltros, sOpDBSel: pchar; cOpBlueField, cOpNormal : TColor; iEmpresa, iOp: integer; bOpModal: boolean = false): pchar;
begin
  Result := '';
  with TfmSGC_BANCOS.Create(Application) do
  begin;
    bModal := bOpModal;
...
procedure TfmSGC_BANCOS.btTransportaClick(Sender: TObject);
begin
  if bModal then
  begin
    sRetorno := cdsCOD_BANCO.AsString+';'
               +cdsNOME.AsString+';'
               ;
...
procedure TfmSGC_BANCOS.FormCreate(Sender: TObject);
begin
  bDBAberto := false;
...
procedure TfmSGC_BANCOS.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfmSGC_BANCOS.HabilitaControles;
begin
...
procedure TfmSGC_BANCOS.btConsultaClick(Sender: TObject);
begin
  if not bDBAberto
  then
  begin
    dtmCon := TdtmConexao.Create(fmSGC_BANCOS);
  ...
  begin
    ...
    SQL.Clear;
    SQL.Add(' select * from SGC_BANCOS ');
    SQL.Add(' where ID > 0 ');
    if trim(edtConsulta.Text) <> ''
    then SQL.Add(' and COD_BANCO = '''+trim(edtConsulta.Text)+''' ');
...
procedure TfmSGC_BANCOS.cdsAfterPost(DataSet: TDataSet);
var
  td : TTransactionDesc;
begin
...
procedure TfmSGC_BANCOS.cdsBeforePost(DataSet: TDataSet);
begin
...
procedure TfmSGC_BANCOS.cdsAfterEdit(DataSet: TDataSet);
begin
...
procedure TfmSGC_BANCOS.cdsAfterInsert(DataSet: TDataSet);
begin
  cdsUSU_INCLUI.AsString := sUsuario;
  HabilitaControles;
end;

procedure TfmSGC_BANCOS.dspUpdateError(Sender: TObject;
...
procedure TfmSGC_BANCOS.BeforeUpdateRecord(Sender: TObject; SourceDS: TDataSet;
...
procedure TfmSGC_BANCOS.dbgConsultaDblClick(Sender: TObject);
begin
  if bModal
  and not cds.IsEmpty
...
procedure TfmSGC_BANCOS.cdsNewRecord(DataSet: TDataSet);
begin
  cdsID.ReadOnly := false;
  HabilitaControles;
end;

procedure TfmSGC_BANCOS.cdsAfterCancel(DataSet: TDataSet);
begin
  cdsID.ReadOnly := true;
  HabilitaControles;
end;

procedure TfmSGC_BANCOS.cdsAfterDelete(DataSet: TDataSet);
var
  td : TTransactionDesc;
begin
  td.TransactionID := Cardinal(FormatDateTime('ddmmyyyy',date)+FormatDateTime('hhnnss',time));
...
procedure TfmSGC_BANCOS.btSalvaClick(Sender: TObject);
begin
  if Pode_Salvar
  then cds.Post;
end;

procedure TfmSGC_BANCOS.btImprimeClick(Sender: TObject);
begin
 if (not cds.Active) or cds.IsEmpty then exit;
  ...
  begin
    ...
    DefinicaoCampos.Add('D0;20;D;;COD_BANCO;Cód. Banco');
...
procedure TfmSGC_BANCOS.btIncluiClick(Sender: TObject);
begin
...
procedure TfmSGC_BANCOS.btDesisteClick(Sender: TObject);
begin
...
procedure TfmSGC_BANCOS.btExcluiClick(Sender: TObject);
begin
...
procedure TfmSGC_BANCOS.btSaiClick(Sender: TObject);
begin
  Close;
end;

procedure TfmSGC_BANCOS.tsConsultaShow(Sender: TObject);
begin
  if cds.State in [dsInsert, dsEdit]
  then cds.Cancel;
...
procedure TfmSGC_BANCOS.edtConsultaChange(Sender: TObject);
begin
if cds.Active then cds.Close;

end;

procedure TfmSGC_BANCOS.edtConsultaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmSGC_BANCOS.edtConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = chr(vk_return) then btConsultaClick(self);
end;

procedure TfmSGC_BANCOS.dbgConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfmSGC_BANCOS.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfmSGC_BANCOS.Direitos;
var
  s : string;
begin
  bCad := false;
...
function TfmSGC_BANCOS.Pode_Salvar: boolean;
begin
  result := false;
