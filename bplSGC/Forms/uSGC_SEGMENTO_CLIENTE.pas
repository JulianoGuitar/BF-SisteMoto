...
uses
  ...
  Buttons, bfEdit, bfdbEdit, DBGrids, bfdbMemo, RelVisualBlue, bfdbEditButton, dbxpress;

type
  TfmSGC_SEGMENTO_CLIENTE = class(TForm)
    cds: TClientDataSet;
...
var
  fmSGC_SEGMENTO_CLIENTE: TfmSGC_SEGMENTO_CLIENTE;
const
  sCodProg : string = 'SGC009';
...
function SGC009(sOpUsuario, sOpLocal, sOpFiltros, sOpDBSel: pchar; cOpBlueField, cOpNormal : TColor; iEmpresa, iOp: integer; bOpModal: boolean = false): pchar;
begin
  Result := '';
  with TfmSGC_SEGMENTO_CLIENTE.Create(Application) do
  begin;
    bModal := bOpModal;
...
procedure TfmSGC_SEGMENTO_CLIENTE.btTransportaClick(Sender: TObject);
begin
  if bModal then
  begin
...
procedure TfmSGC_SEGMENTO_CLIENTE.FormCreate(Sender: TObject);
begin
  bDBAberto := false;
...
procedure TfmSGC_SEGMENTO_CLIENTE.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  ...
  if not bModal then
  begin
    fmSGC_SEGMENTO_CLIENTE := nil;
    Action := caFree;
  end;
end;

procedure TfmSGC_SEGMENTO_CLIENTE.HabilitaControles;
begin
...
procedure TfmSGC_SEGMENTO_CLIENTE.btConsultaClick(Sender: TObject);
begin
  if not bDBAberto
  then
  begin
    dtmCon := TdtmConexao.Create(fmSGC_SEGMENTO_CLIENTE);
  ...
  begin
    ...
    SQL.Clear;
    SQL.Add(' select s.* from SGC_SEGMENTO_CLIENTE s ');
    SQL.Add(' where s.ID > 0 ');
    if trim(edtConsulta.Text) <> ''
    then SQL.Add(' and s.ID = '''+trim(edtConsulta.Text)+''' ');
    if trim(edtConsultaNOME.Text) <> ''
    then SQL.Add(' and s.DESCRICAO like ''%'+trim(edtConsultaNOME.Text)+'%'' ');
    if ckbInativoCons.Checked then
    SQL.Add(' and s.INATIVO = ''S'' ')
    else
    SQL.Add(' and s.INATIVO <> ''S'' ');
  end;
...
procedure TfmSGC_SEGMENTO_CLIENTE.cdsAfterPost(DataSet: TDataSet);
var
  td : TTransactionDesc;
begin
...
procedure TfmSGC_SEGMENTO_CLIENTE.cdsBeforePost(DataSet: TDataSet);
begin
...
procedure TfmSGC_SEGMENTO_CLIENTE.cdsIDGetText(Sender: TField; var Text: string;
  DisplayText: Boolean);
begin
...
procedure TfmSGC_SEGMENTO_CLIENTE.cdsAfterEdit(DataSet: TDataSet);
begin
...
procedure TfmSGC_SEGMENTO_CLIENTE.cdsAfterInsert(DataSet: TDataSet);
begin
  cdsUSU_INCLUI.AsString := sUsuario;
  HabilitaControles;
end;

procedure TfmSGC_SEGMENTO_CLIENTE.dspUpdateError(Sender: TObject;
...
procedure TfmSGC_SEGMENTO_CLIENTE.BeforeUpdateRecord(Sender: TObject; SourceDS: TDataSet;
...
procedure TfmSGC_SEGMENTO_CLIENTE.dbgConsultaDblClick(Sender: TObject);
begin
  if bModal
  and not cds.IsEmpty
...
procedure TfmSGC_SEGMENTO_CLIENTE.cdsNewRecord(DataSet: TDataSet);
begin
  cdsID.ReadOnly := false;
  HabilitaControles;
end;

procedure TfmSGC_SEGMENTO_CLIENTE.cdsAfterCancel(DataSet: TDataSet);
begin
  cdsID.ReadOnly := true;
  HabilitaControles;
end;

procedure TfmSGC_SEGMENTO_CLIENTE.cdsAfterDelete(DataSet: TDataSet);
var
  td : TTransactionDesc;
begin
  td.TransactionID := Cardinal(FormatDateTime('ddmmyyyy',date)+FormatDateTime('hhnnss',time));
...
procedure TfmSGC_SEGMENTO_CLIENTE.btSalvaClick(Sender: TObject);
begin
  if Pode_Salvar
  then cds.Post;
end;

procedure TfmSGC_SEGMENTO_CLIENTE.btImprimeClick(Sender: TObject);
begin
...
procedure TfmSGC_SEGMENTO_CLIENTE.btIncluiClick(Sender: TObject);
begin
...
procedure TfmSGC_SEGMENTO_CLIENTE.btDesisteClick(Sender: TObject);
begin
...
procedure TfmSGC_SEGMENTO_CLIENTE.btExcluiClick(Sender: TObject);
begin
...
procedure TfmSGC_SEGMENTO_CLIENTE.btSaiClick(Sender: TObject);
begin
  Close;
end;

procedure TfmSGC_SEGMENTO_CLIENTE.tsConsultaShow(Sender: TObject);
begin
  if cds.State in [dsInsert, dsEdit]
  then cds.Cancel;
...
procedure TfmSGC_SEGMENTO_CLIENTE.edtConsultaChange(Sender: TObject);
begin
if cds.Active then cds.Close;

end;

procedure TfmSGC_SEGMENTO_CLIENTE.edtConsultaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmSGC_SEGMENTO_CLIENTE.edtConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = chr(vk_return) then btConsultaClick(self);
end;

procedure TfmSGC_SEGMENTO_CLIENTE.dbgConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfmSGC_SEGMENTO_CLIENTE.
...
procedure TfmSGC_SEGMENTO_CLIENTE.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfmSGC_SEGMENTO_CLIENTE.Direitos;
var
  s : string;
begin
  bCad := false;
...
function TfmSGC_SEGMENTO_CLIENTE.Pode_Salvar: boolean;
begin
  result := false;
