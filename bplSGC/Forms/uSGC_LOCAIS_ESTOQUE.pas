...
type
  TfmSGC_LOCAIS_ESTOQUE = class(TForm)
    cds: TClientDataSet;
...
var
  fmSGC_LOCAIS_ESTOQUE: TfmSGC_LOCAIS_ESTOQUE;
const
  sCodProg : string = 'SGC008';
...
function SGC008(sOpUsuario, sOpLocal, sOpFiltros, sOpDBSel: pchar; cOpBlueField, cOpNormal : TColor; iEmpresa, iOp: integer; bOpModal: boolean = false): pchar;
begin
  Result := '';
  with TfmSGC_LOCAIS_ESTOQUE.Create(Application) do
  begin;
    bModal := bOpModal;
...
procedure TfmSGC_LOCAIS_ESTOQUE.btTransportaClick(Sender: TObject);
begin
  if bModal then
  begin
...
procedure TfmSGC_LOCAIS_ESTOQUE.FormCreate(Sender: TObject);
begin
  bDBAberto := false;
...
procedure TfmSGC_LOCAIS_ESTOQUE.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  ...
  if not bModal then
  begin
    fmSGC_LOCAIS_ESTOQUE := nil;
    Action := caFree;
  end;
end;

procedure TfmSGC_LOCAIS_ESTOQUE.HabilitaControles;
begin
...
procedure TfmSGC_LOCAIS_ESTOQUE.btConsultaClick(Sender: TObject);
begin
  if not bDBAberto
  then
  begin
    dtmCon := TdtmConexao.Create(fmSGC_LOCAIS_ESTOQUE);
  ...
  begin
    ...
    SQL.Clear;
    SQL.Add(' select * from SGC_LOCAIS_ESTOQUE ');
    SQL.Add(' where ID > 0 ');
...
procedure TfmSGC_LOCAIS_ESTOQUE.cdsAfterPost(DataSet: TDataSet);
var
  td : TTransactionDesc;
begin
...
procedure TfmSGC_LOCAIS_ESTOQUE.cdsBeforePost(DataSet: TDataSet);
begin
...
procedure TfmSGC_LOCAIS_ESTOQUE.cdsIDGetText(Sender: TField; var Text: string;
  DisplayText: Boolean);
begin
...
procedure TfmSGC_LOCAIS_ESTOQUE.cdsAfterEdit(DataSet: TDataSet);
begin
...
procedure TfmSGC_LOCAIS_ESTOQUE.cdsAfterInsert(DataSet: TDataSet);
begin
  cdsUSU_INCLUI.AsString := sUsuario;
  HabilitaControles;
end;

procedure TfmSGC_LOCAIS_ESTOQUE.dspUpdateError(Sender: TObject;
...
procedure TfmSGC_LOCAIS_ESTOQUE.BeforeUpdateRecord(Sender: TObject; SourceDS: TDataSet;
...
procedure TfmSGC_LOCAIS_ESTOQUE.dbgConsultaDblClick(Sender: TObject);
begin
  if bModal
  and not cds.IsEmpty
...
procedure TfmSGC_LOCAIS_ESTOQUE.cdsNewRecord(DataSet: TDataSet);
begin
  cdsID.ReadOnly := false;
  HabilitaControles;
end;

procedure TfmSGC_LOCAIS_ESTOQUE.cdsAfterCancel(DataSet: TDataSet);
begin
  cdsID.ReadOnly := true;
  HabilitaControles;
end;

procedure TfmSGC_LOCAIS_ESTOQUE.cdsAfterDelete(DataSet: TDataSet);
var
  td : TTransactionDesc;
begin
  td.TransactionID := Cardinal(FormatDateTime('ddmmyyyy',date)+FormatDateTime('hhnnss',time));
...
procedure TfmSGC_LOCAIS_ESTOQUE.btSalvaClick(Sender: TObject);
begin
  if Pode_Salvar
  then cds.Post;
end;

procedure TfmSGC_LOCAIS_ESTOQUE.btImprimeClick(Sender: TObject);
begin
...
procedure TfmSGC_LOCAIS_ESTOQUE.btIncluiClick(Sender: TObject);
begin
...
procedure TfmSGC_LOCAIS_ESTOQUE.btDesisteClick(Sender: TObject);
begin
...
procedure TfmSGC_LOCAIS_ESTOQUE.btExcluiClick(Sender: TObject);
begin
...
procedure TfmSGC_LOCAIS_ESTOQUE.btSaiClick(Sender: TObject);
begin
  Close;
end;

procedure TfmSGC_LOCAIS_ESTOQUE.tsConsultaShow(Sender: TObject);
begin
  if cds.State in [dsInsert, dsEdit]
  then cds.Cancel;
...
procedure TfmSGC_LOCAIS_ESTOQUE.edtConsultaChange(Sender: TObject);
begin
if cds.Active then cds.Close;

end;

procedure TfmSGC_LOCAIS_ESTOQUE.edtConsultaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmSGC_LOCAIS_ESTOQUE.edtConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = chr(vk_return) then btConsultaClick(self);
end;

procedure TfmSGC_LOCAIS_ESTOQUE.dbgConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfmSGC_LOCAIS_ESTOQUE.
...
procedure TfmSGC_LOCAIS_ESTOQUE.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfmSGC_LOCAIS_ESTOQUE.Direitos;
var
  s : string;
begin
  bCad := false;
...
function TfmSGC_LOCAIS_ESTOQUE.Pode_Salvar: boolean;
begin
  result := false;
