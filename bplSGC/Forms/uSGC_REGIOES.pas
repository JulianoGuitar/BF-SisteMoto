...
type
  TfmSGC_REGIOES = class(TForm)
    ...
    qrVLR_FRETE: TFMTBCDField;
    cdsVLR_FRETE: TFMTBCDField;
    Label3: TLabel;
    DBEdit1: TbfdbEdit;
...
var
  fmSGC_REGIOES: TfmSGC_REGIOES;
const
  sCodProg : string = 'SGC013';
...
function SGC013(sOpUsuario, sOpLocal, sOpFiltros, sOpDBSel: pchar; cOpBlueField, cOpNormal : TColor; iEmpresa, iOp: integer; bOpModal: boolean = false): pchar;
begin
  Result := '';
  with TfmSGC_REGIOES.Create(Application) do
  begin;
    bModal := bOpModal;
...
procedure TfmSGC_REGIOES.btTransportaClick(Sender: TObject);
begin
  if bModal then
  begin
...
procedure TfmSGC_REGIOES.FormCreate(Sender: TObject);
begin
  bDBAberto := false;
...
procedure TfmSGC_REGIOES.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfmSGC_REGIOES.HabilitaControles;
begin
...
procedure TfmSGC_REGIOES.btConsultaClick(Sender: TObject);
begin
  if not bDBAberto
  then
  begin
    dtmCon := TdtmConexao.Create(fmSGC_REGIOES);
  ...
  begin
    ...
    SQL.Clear;
    SQL.Add(' select * from SGC_REGIOES ');
    SQL.Add(' where ID > 0 ');
...
procedure TfmSGC_REGIOES.cdsAfterPost(DataSet: TDataSet);
var
  td : TTransactionDesc;
begin
...
procedure TfmSGC_REGIOES.cdsBeforePost(DataSet: TDataSet);
begin
...
procedure TfmSGC_REGIOES.cdsIDGetText(Sender: TField; var Text: string;
  DisplayText: Boolean);
begin
...
procedure TfmSGC_REGIOES.cdsAfterEdit(DataSet: TDataSet);
begin
...
procedure TfmSGC_REGIOES.cdsAfterInsert(DataSet: TDataSet);
begin
  cdsUSU_INCLUI.AsString := sUsuario;
  HabilitaControles;
end;

procedure TfmSGC_REGIOES.dspUpdateError(Sender: TObject;
...
procedure TfmSGC_REGIOES.BeforeUpdateRecord(Sender: TObject; SourceDS: TDataSet;
...
procedure TfmSGC_REGIOES.dbgConsultaDblClick(Sender: TObject);
begin
  if bModal
  and not cds.IsEmpty
...
procedure TfmSGC_REGIOES.cdsNewRecord(DataSet: TDataSet);
begin
  cdsID.ReadOnly := false;
  HabilitaControles;
end;

procedure TfmSGC_REGIOES.cdsAfterCancel(DataSet: TDataSet);
begin
  cdsID.ReadOnly := true;
  HabilitaControles;
end;

procedure TfmSGC_REGIOES.cdsAfterDelete(DataSet: TDataSet);
var
  td : TTransactionDesc;
begin
  td.TransactionID := Cardinal(FormatDateTime('ddmmyyyy',date)+FormatDateTime('hhnnss',time));
...
procedure TfmSGC_REGIOES.btSalvaClick(Sender: TObject);
begin
  if Pode_Salvar
  then cds.Post;
end;

procedure TfmSGC_REGIOES.btImprimeClick(Sender: TObject);
begin
...
procedure TfmSGC_REGIOES.btIncluiClick(Sender: TObject);
begin
...
procedure TfmSGC_REGIOES.btDesisteClick(Sender: TObject);
begin
...
procedure TfmSGC_REGIOES.btExcluiClick(Sender: TObject);
begin
...
procedure TfmSGC_REGIOES.btSaiClick(Sender: TObject);
begin
  Close;
end;

procedure TfmSGC_REGIOES.tsConsultaShow(Sender: TObject);
begin
  if cds.State in [dsInsert, dsEdit]
  then cds.Cancel;
...
procedure TfmSGC_REGIOES.edtConsultaChange(Sender: TObject);
begin
if cds.Active then cds.Close;

end;

procedure TfmSGC_REGIOES.edtConsultaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmSGC_REGIOES.edtConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = chr(vk_return) then btConsultaClick(self);
end;

procedure TfmSGC_REGIOES.dbgConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfmSGC_REGIOES.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfmSGC_REGIOES.Direitos;
var
  s : string;
begin
  bCad := false;
...
function TfmSGC_REGIOES.Pode_Salvar: boolean;
begin
  result := false;
