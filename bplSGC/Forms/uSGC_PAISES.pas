...
type
  TfmSGC_PAISES = class(TForm)
    ...
    cdsDT_CRIACAO: TStringField;
    cdsDT_EXTINCAO: TStringField;
...
var
  fmSGC_PAISES: TfmSGC_PAISES;
const
  sCodProg : string = 'SGC006';
...
function SGC006(sOpUsuario, sOpLocal, sOpFiltros, sOpDBSel: pchar; cOpBlueField, cOpNormal : TColor; iEmpresa, iOp: integer; bOpModal: boolean = false): pchar;
begin
  Result := '';
  with TfmSGC_PAISES.Create(Application) do
  begin;
    bModal := bOpModal;
...
procedure TfmSGC_PAISES.btTransportaClick(Sender: TObject);
begin
  if bModal then
  begin
...
procedure TfmSGC_PAISES.FormCreate(Sender: TObject);
begin
  bDBAberto := false;
...
procedure TfmSGC_PAISES.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfmSGC_PAISES.HabilitaControles;
begin
...
procedure TfmSGC_PAISES.btConsultaClick(Sender: TObject);
begin
  if not bDBAberto
  then
  begin
    dtmCon := TdtmConexao.Create(fmSGC_PAISES);
    bDBAberto := AbreDataModule(dtmCon, sPath_Local, sBancoDeDados);
    dsp.DataSet := dtmCon.qrTela;
  end;

  if not bDBAberto then exit;
  with  dtmCon.qrTela do
  begin
    SQLConnection := dtmCon.SQLDB;
    if active then close;
    SQL.Clear;
    SQL.Add(' select * from SGC_PAISES ');
    SQL.Add(' where ID > 0');
...
procedure TfmSGC_PAISES.cdsBeforePost(DataSet: TDataSet);
begin
...
procedure TfmSGC_PAISES.cdsAfterEdit(DataSet: TDataSet);
begin
...
procedure TfmSGC_PAISES.cdsAfterInsert(DataSet: TDataSet);
begin
  cdsUSU_INCLUI.AsString := sUsuario;
  HabilitaControles;
end;

procedure TfmSGC_PAISES.dspUpdateError(Sender: TObject;
...
procedure TfmSGC_PAISES.BeforeUpdateRecord(Sender: TObject; SourceDS: TDataSet;
...
procedure TfmSGC_PAISES.dbgConsultaDblClick(Sender: TObject);
begin
  if bModal
  and not cds.IsEmpty
...
procedure TfmSGC_PAISES.cdsNewRecord(DataSet: TDataSet);
begin
  //cdsID.ReadOnly := false;
  HabilitaControles;
end;

procedure TfmSGC_PAISES.cdsAfterCancel(DataSet: TDataSet);
begin
  cdsID.ReadOnly := true;
  HabilitaControles;
end;

procedure TfmSGC_PAISES.cdsAfterDelete(DataSet: TDataSet);
begin
  Salvar;
end;

procedure TfmSGC_PAISES.btSalvaClick(Sender: TObject);
begin
  if Pode_Salvar
  then
  begin
...
procedure TfmSGC_PAISES.btImprimeClick(Sender: TObject);
begin
 if (not cds.Active) or cds.IsEmpty then exit;
  ...
  begin
    ...
    Cabecalho3Centro := edtConsulta.Text;
    Cabecalho2Centro := '';

    Cabecalhos.Clear;

    Limpa_Campos;
    DefinicaoCampos.Add('D0;20;E;;ID;ID');
...
procedure TfmSGC_PAISES.btIncluiClick(Sender: TObject);
begin
...
procedure TfmSGC_PAISES.btDesisteClick(Sender: TObject);
begin
...
procedure TfmSGC_PAISES.btExcluiClick(Sender: TObject);
begin
...
procedure TfmSGC_PAISES.btSaiClick(Sender: TObject);
begin
  Close;
end;

procedure TfmSGC_PAISES.tsConsultaShow(Sender: TObject);
begin
  if cds.State in [dsInsert, dsEdit]
  then cds.Cancel;
...
procedure TfmSGC_PAISES.edtConsultaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmSGC_PAISES.edtConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = chr(vk_return) then btConsultaClick(self);
end;

procedure TfmSGC_PAISES.dbgConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfmSGC_PAISES.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfmSGC_PAISES.Direitos;
var
  s : string;
begin
  bCad := false;
...
function TfmSGC_PAISES.Pode_Salvar: boolean;
begin
  result := false;
...
procedure TfmSGC_PAISES.Salvar;
var
  td : TTransactionDesc;
begin
...
procedure TfmSGC_PAISES.cdsIDGetText(Sender: TField; var Text: String;
  DisplayText: Boolean);
begin
