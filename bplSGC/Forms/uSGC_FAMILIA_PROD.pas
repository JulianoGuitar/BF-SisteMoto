...
type
  TfmSGC_FAMILIA_PROD = class(TForm)
    cds: TClientDataSet;
...
var
  fmSGC_FAMILIA_PROD: TfmSGC_FAMILIA_PROD;
const
  sCodProg : string = 'SGC021';
...
function SGC021(sOpUsuario, sOpLocal, sOpFiltros, sOpDBSel: pchar; cOpBlueField, cOpNormal : TColor; iEmpresa, iOp: integer; bOpModal: boolean = false): pchar;
begin
  Result := '';
  with TfmSGC_FAMILIA_PROD.Create(Application) do
  begin;
    bModal := bOpModal;
...
procedure TfmSGC_FAMILIA_PROD.btTransportaClick(Sender: TObject);
begin
  if bModal then
  begin
...
procedure TfmSGC_FAMILIA_PROD.FormCreate(Sender: TObject);
begin
  bDBAberto := false;
...
procedure TfmSGC_FAMILIA_PROD.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  ...
  if not bModal then
  begin
    fmSGC_FAMILIA_PROD := nil;
    Action := caFree;
  end;
end;

procedure TfmSGC_FAMILIA_PROD.HabilitaControles;
begin
...
procedure TfmSGC_FAMILIA_PROD.btConsultaClick(Sender: TObject);
begin
  if not bDBAberto
  then
  begin
    dtmCon := TdtmConexao.Create(fmSGC_FAMILIA_PROD);
  ...
  begin
    ...
    SQL.Clear;
    SQL.Add(' select * from SGC_FAMILIA_PROD ');
    SQL.Add(' where ID > 0 ');
...
procedure TfmSGC_FAMILIA_PROD.cdsAfterPost(DataSet: TDataSet);
var
  td : TTransactionDesc;
begin
...
procedure TfmSGC_FAMILIA_PROD.cdsBeforePost(DataSet: TDataSet);
begin
...
procedure TfmSGC_FAMILIA_PROD.cdsIDGetText(Sender: TField; var Text: string;
  DisplayText: Boolean);
begin
...
procedure TfmSGC_FAMILIA_PROD.cdsAfterEdit(DataSet: TDataSet);
begin
...
procedure TfmSGC_FAMILIA_PROD.cdsAfterInsert(DataSet: TDataSet);
begin
  cdsUSU_INCLUI.AsString := sUsuario;
  HabilitaControles;
end;

procedure TfmSGC_FAMILIA_PROD.dspUpdateError(Sender: TObject;
...
procedure TfmSGC_FAMILIA_PROD.BeforeUpdateRecord(Sender: TObject; SourceDS: TDataSet;
...
procedure TfmSGC_FAMILIA_PROD.dbgConsultaDblClick(Sender: TObject);
begin
  if bModal
  and not cds.IsEmpty
...
procedure TfmSGC_FAMILIA_PROD.cdsNewRecord(DataSet: TDataSet);
begin
  cdsID.ReadOnly := false;
  HabilitaControles;
end;

procedure TfmSGC_FAMILIA_PROD.cdsAfterCancel(DataSet: TDataSet);
begin
  cdsID.ReadOnly := true;
  HabilitaControles;
end;

procedure TfmSGC_FAMILIA_PROD.cdsAfterDelete(DataSet: TDataSet);
var
  td : TTransactionDesc;
begin
  td.TransactionID := Cardinal(FormatDateTime('ddmmyyyy',date)+FormatDateTime('hhnnss',time));
...
procedure TfmSGC_FAMILIA_PROD.btSalvaClick(Sender: TObject);
begin
  if Pode_Salvar
  then cds.Post;
end;

procedure TfmSGC_FAMILIA_PROD.btImprimeClick(Sender: TObject);
begin
...
procedure TfmSGC_FAMILIA_PROD.btIncluiClick(Sender: TObject);
begin
...
procedure TfmSGC_FAMILIA_PROD.btDesisteClick(Sender: TObject);
begin
...
procedure TfmSGC_FAMILIA_PROD.btExcluiClick(Sender: TObject);
begin
...
procedure TfmSGC_FAMILIA_PROD.btSaiClick(Sender: TObject);
begin
  Close;
end;

procedure TfmSGC_FAMILIA_PROD.tsConsultaShow(Sender: TObject);
begin
  if cds.State in [dsInsert, dsEdit]
  then cds.Cancel;
...
procedure TfmSGC_FAMILIA_PROD.edtConsultaChange(Sender: TObject);
begin
if cds.Active then cds.Close;

end;

procedure TfmSGC_FAMILIA_PROD.edtConsultaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmSGC_FAMILIA_PROD.edtConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = chr(vk_return) then btConsultaClick(self);
end;

procedure TfmSGC_FAMILIA_PROD.dbgConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfmSGC_FAMILIA_PROD.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfmSGC_FAMILIA_PROD.Direitos;
var
  s : string;
begin
  bCad := false;
...
function TfmSGC_FAMILIA_PROD.Pode_Salvar: boolean;
begin
  result := false;
