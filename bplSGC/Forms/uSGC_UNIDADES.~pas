...
var
  fmSGC_UNIDADES: TfmSGC_UNIDADES;
const
  sCodProg : string = 'SGC014';
...
function SGC014(sOpUsuario, sOpLocal, sOpFiltros, sOpDBSel: pchar; cOpBlueField, cOpNormal : TColor; iEmpresa, iOp: integer; bOpModal: boolean = false): pchar;
begin
  Result := '';
  with TfmSGC_UNIDADES.Create(Application) do
  begin;
    bModal := bOpModal;
...
procedure TfmSGC_UNIDADES.btTransportaClick(Sender: TObject);
begin
  if bModal then
  begin
    sRetorno := cdsUNIDADE.AsString+';'
               +cdsNOME.AsString+';'
               ;
...
procedure TfmSGC_UNIDADES.FormCreate(Sender: TObject);
begin
  bDBAberto := false;
...
procedure TfmSGC_UNIDADES.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfmSGC_UNIDADES.HabilitaControles;
begin
...
procedure TfmSGC_UNIDADES.btConsultaClick(Sender: TObject);
begin
  if not bDBAberto
  then
  begin
    dtmCon := TdtmConexao.Create(fmSGC_UNIDADES);
  ...
  begin
    ...
    SQL.Clear;
    SQL.Add(' select * from SGC_UNIDADES ');
    SQL.Add(' where ID > 0 ');
    if trim(edtConsulta.Text) <> ''
    then SQL.Add(' and UNIDADE = '''+trim(edtConsulta.Text)+''' ');
    if trim(edtConsultaNOME.Text) <> ''
    then SQL.Add(' and NOME like ''%'+trim(edtConsultaNOME.Text)+'%'' ');
...
procedure TfmSGC_UNIDADES.cdsAfterPost(DataSet: TDataSet);
var
  td : TTransactionDesc;
begin
...
procedure TfmSGC_UNIDADES.cdsBeforePost(DataSet: TDataSet);
begin
...
procedure TfmSGC_UNIDADES.cdsAfterEdit(DataSet: TDataSet);
begin
...
procedure TfmSGC_UNIDADES.cdsAfterInsert(DataSet: TDataSet);
begin
  cdsUSU_INCLUI.AsString := sUsuario;
  HabilitaControles;
end;

procedure TfmSGC_UNIDADES.dspUpdateError(Sender: TObject;
...
procedure TfmSGC_UNIDADES.BeforeUpdateRecord(Sender: TObject; SourceDS: TDataSet;
...
procedure TfmSGC_UNIDADES.dbgConsultaDblClick(Sender: TObject);
begin
  if bModal
  and not cds.IsEmpty
...
procedure TfmSGC_UNIDADES.cdsNewRecord(DataSet: TDataSet);
begin
  cdsID.ReadOnly := false;
  HabilitaControles;
end;

procedure TfmSGC_UNIDADES.cdsAfterCancel(DataSet: TDataSet);
begin
  cdsID.ReadOnly := true;
  HabilitaControles;
end;

procedure TfmSGC_UNIDADES.cdsAfterDelete(DataSet: TDataSet);
var
  td : TTransactionDesc;
begin
  td.TransactionID := Cardinal(FormatDateTime('ddmmyyyy',date)+FormatDateTime('hhnnss',time));
...
procedure TfmSGC_UNIDADES.btSalvaClick(Sender: TObject);
begin
  if Pode_Salvar
  then cds.Post;
end;

procedure TfmSGC_UNIDADES.btImprimeClick(Sender: TObject);
begin
 if (not cds.Active) or cds.IsEmpty then exit;
  ...
  begin
    ...
    DefinicaoCampos.Add('D0;20;D;;UNIDADE;Unidade');
...
procedure TfmSGC_UNIDADES.btIncluiClick(Sender: TObject);
begin
...
procedure TfmSGC_UNIDADES.btDesisteClick(Sender: TObject);
begin
...
procedure TfmSGC_UNIDADES.btExcluiClick(Sender: TObject);
begin
...
procedure TfmSGC_UNIDADES.btSaiClick(Sender: TObject);
begin
  Close;
end;

procedure TfmSGC_UNIDADES.tsConsultaShow(Sender: TObject);
begin
  if cds.State in [dsInsert, dsEdit]
  then cds.Cancel;
...
procedure TfmSGC_UNIDADES.edtConsultaChange(Sender: TObject);
begin
if cds.Active then cds.Close;

end;

procedure TfmSGC_UNIDADES.edtConsultaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmSGC_UNIDADES.edtConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = chr(vk_return) then btConsultaClick(self);
end;

procedure TfmSGC_UNIDADES.dbgConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfmSGC_UNIDADES.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfmSGC_UNIDADES.Direitos;
var
  s : string;
begin
  bCad := false;
...
function TfmSGC_UNIDADES.Pode_Salvar: boolean;
begin
  result := false;
