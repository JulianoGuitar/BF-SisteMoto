...
type
  TfmSGC_TRANSPORTADORAS = class(TForm)
    cds: TClientDataSet;
...
var
  fmSGC_TRANSPORTADORAS: TfmSGC_TRANSPORTADORAS;
const
  sCodProg : string = 'SGC019';
...
function SGC019(sOpUsuario, sOpLocal, sOpFiltros, sOpDBSel: pchar; cOpBlueField, cOpNormal : TColor; iEmpresa, iOp: integer; bOpModal: boolean = false): pchar;
begin
  Result := '';
  with TfmSGC_TRANSPORTADORAS.Create(Application) do
  begin;
    bModal := bOpModal;
...
procedure TfmSGC_TRANSPORTADORAS.btTransportaClick(Sender: TObject);
begin
  if bModal then
  begin
...
function TfmSGC_TRANSPORTADORAS.ExisteCNPJ: boolean;
begin
  result := false;
...
procedure TfmSGC_TRANSPORTADORAS.FormCreate(Sender: TObject);
begin
  bDBAberto := false;
  ...
  if cdsINATIVO_TRANSP.AsString = 'S' then
  ckbInativo.Checked
end;

procedure TfmSGC_TRANSPORTADORAS.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  ...
  if not bModal then
  begin
    fmSGC_TRANSPORTADORAS := nil;
    Action := caFree;
  end;  
end;

procedure TfmSGC_TRANSPORTADORAS.HabilitaControles;
begin
...
procedure TfmSGC_TRANSPORTADORAS.edtCodCidadeButtonClick(Sender: TObject);
var
  s : string;
begin
  if cds.Active
...
procedure TfmSGC_TRANSPORTADORAS.edtCodCidadeChange(Sender: TObject);
begin
  if edtCodCidade.Focused then edtNomeCidade.Clear;
end;

procedure TfmSGC_TRANSPORTADORAS.edtCodCidadeExit(Sender: TObject);
begin
//  ChangeExit(DBEdit3);
...
procedure TfmSGC_TRANSPORTADORAS.edtCodPaisButtonClick(Sender: TObject);
var
  s : string;
begin
  if cds.Active
...
procedure TfmSGC_TRANSPORTADORAS.edtCodPaisChange(Sender: TObject);
begin
  if edtCodPais.Focused then edtNomePais.Clear;
end;

procedure TfmSGC_TRANSPORTADORAS.edtCodPaisExit(Sender: TObject);
begin
//  ChangeExit(DBEdit3);
...
procedure TfmSGC_TRANSPORTADORAS.bfdbGrid2DblClick(Sender: TObject);
var
  s : string;
begin
  if cds.Active
...
procedure TfmSGC_TRANSPORTADORAS.btConsultaClick(Sender: TObject);
begin
  if not bDBAberto
  then
  begin
    dtmCon := TdtmConexao.Create(fmSGC_TRANSPORTADORAS);
  ...
  begin
    ...
    SQL.Add(' where e.ID > 0 and TRANSPORTADORA = 1 ');
    ...
    SQL.Add(' and e.INATIVO_TRANSP = ''S'' ')
    else
    SQL.Add(' and e.INATIVO_TRANSP <> ''S'' ');
  end;
...
procedure TfmSGC_TRANSPORTADORAS.btConsultaCNPJClick(Sender: TObject);
var s : string;
begin
...
procedure TfmSGC_TRANSPORTADORAS.cdsAfterPost(DataSet: TDataSet);
var
  td : TTransactionDesc;
begin
...
procedure TfmSGC_TRANSPORTADORAS.cdsBeforePost(DataSet: TDataSet);
begin
  cdsIDALTERADO.AsString := cdsIDALT.AsString;
  if ckbInativo.Checked then
  cdsINATIVO_TRANSP.AsString := 'S';
  cdsTRANSPORTADORA.AsString := '1';
end;

procedure TfmSGC_TRANSPORTADORAS.cdsAfterEdit(DataSet: TDataSet);
begin
...
procedure TfmSGC_TRANSPORTADORAS.cdsAfterInsert(DataSet: TDataSet);
begin
  cdsUSU_INCLUI.AsString := sUsuario;
  HabilitaControles;
end;

procedure TfmSGC_TRANSPORTADORAS.dspUpdateError(Sender: TObject;
...
procedure TfmSGC_TRANSPORTADORAS.AbreContatos;
begin
  with qrContatos do
  begin
    SQLConnection := dtmCon.SQLDB;
...
procedure TfmSGC_TRANSPORTADORAS.BeforeUpdateRecord(Sender: TObject; SourceDS: TDataSet;
...
procedure TfmSGC_TRANSPORTADORAS.dbgConsultaDblClick(Sender: TObject);
begin
  if bModal
  and not cds.IsEmpty
...
procedure TfmSGC_TRANSPORTADORAS.cdsNewRecord(DataSet: TDataSet);
begin
...
procedure TfmSGC_TRANSPORTADORAS.cdsPESSOAGetText(Sender: TField; var Text: string;
  DisplayText: Boolean);
begin
...
procedure TfmSGC_TRANSPORTADORAS.cdsTIPO_CONTRIBUINTEGetText(Sender: TField;
...
procedure TfmSGC_TRANSPORTADORAS.cbxPessoaExit(Sender: TObject);
begin
...
procedure TfmSGC_TRANSPORTADORAS.cdsAfterCancel(DataSet: TDataSet);
begin
  cdsID.ReadOnly := true;
  HabilitaControles;
end;

procedure TfmSGC_TRANSPORTADORAS.cdsAfterDelete(DataSet: TDataSet);
var
  td : TTransactionDesc;
begin
  td.TransactionID := Cardinal(FormatDateTime('ddmmyyyy',date)+FormatDateTime('hhnnss',time));
...
procedure TfmSGC_TRANSPORTADORAS.btSalvaClick(Sender: TObject);
begin
  if Pode_Salvar
  then cds.Post;
end;

procedure TfmSGC_TRANSPORTADORAS.btImprimeClick(Sender: TObject);
begin
...
procedure TfmSGC_TRANSPORTADORAS.btIncluiClick(Sender: TObject);
begin
...
procedure TfmSGC_TRANSPORTADORAS.btDesisteClick(Sender: TObject);
begin
  if cds.State in [dsInsert, dsEdit]
  then cds.Cancel;
  pgc.ActivePage := tsConsulta;
...
procedure TfmSGC_TRANSPORTADORAS.btExcluiClick(Sender: TObject);
begin
...
procedure TfmSGC_TRANSPORTADORAS.btSaiClick(Sender: TObject);
begin
  Close;
end;

procedure TfmSGC_TRANSPORTADORAS.tsCadastroShow(Sender: TObject);
begin
AbreContatos;
pgcDados.ActivePage := tbDocumentos;
end;

procedure TfmSGC_TRANSPORTADORAS.tsConsultaShow(Sender: TObject);
begin
  if cds.State in [dsInsert, dsEdit]
  then cds.Cancel;
...
procedure TfmSGC_TRANSPORTADORAS.edtConsultaChange(Sender: TObject);
begin
if cds.Active then cds.Close;

end;

procedure TfmSGC_TRANSPORTADORAS.edtConsultaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmSGC_TRANSPORTADORAS.edtConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = chr(vk_return) then btConsultaClick(self);
end;

procedure TfmSGC_TRANSPORTADORAS.dbgConsultaKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfmSGC_TRANSPORTADORAS.dbgConsultaKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key = vk_f9 then edtConsulta.SetFocus;
end;

procedure TfmSGC_TRANSPORTADORAS.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfmSGC_TRANSPORTADORAS.Direitos;
var
  s : string;
begin
  bCad := false;
...
function TfmSGC_TRANSPORTADORAS.Pode_Salvar: boolean;
begin
  result := false;
  ...
  begin
    ...
    begin
       MessageBox(Handle, pchar('Transportador estrangeiro não pode ser Brasileiro.'), 'Aviso', MB_ICONWARNING);
       edtCodPais.SetFocus;
       Exit;
    end;
    ...
    begin
      MessageBox(Handle, pchar('Informe se o Transportador faz parte do Simples Nacional.'), 'Aviso', MB_ICONWARNING);
    ...
    begin
      MessageBox(Handle, pchar('Informe o Tipo de Contribuição do Transportador.'), 'Aviso', MB_ICONWARNING);
...
procedure TfmSGC_TRANSPORTADORAS.TabSheet1ContextPopup(Sender: TObject;
...
procedure TfmSGC_TRANSPORTADORAS.cdsIDGetText(Sender: TField; var Text: String;
  DisplayText: Boolean);
begin
...
procedure TfmSGC_TRANSPORTADORAS.cdsCNPJGetText(Sender: TField; var Text: String;
  DisplayText: Boolean);
begin
  if cds.State in [dsInsert,dsEdit]
...
procedure TfmSGC_TRANSPORTADORAS.cdsEnderecosTIPO_ENDERECOGetText(Sender: TField;
  var Text: string; DisplayText: Boolean);
begin
  if not Sender.IsNull then
  begin
...
procedure TfmSGC_TRANSPORTADORAS.btConsultaModuloClick(Sender: TObject);
begin
end;
