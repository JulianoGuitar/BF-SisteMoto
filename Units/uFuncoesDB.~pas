...
begin
  ...
  begin
    ...
    begin
      Close;
      MessageBox(Application.Handle, pchar('Empresa não habilitada.'), 'Provisual TI', MB_ICONWARNING);
      FechaDataModule(dtmCon);
      Result := '';
      Exit;
    end;
    ...
    begin
      Close;
      MessageBox(Application.Handle, pchar('Há um problema na instalação do programa. (Código 111)'+#13
                                          +'Entre em contato com o suporte técico.'+#13+#13
    ...
    begin
      Close;
      MessageBox(Application.Handle, pchar('Usuário não cadastrado.'), 'Erro', MB_ICONWARNING);
      FechaDataModule(dtmCon);
      Result := LoginDB(u,s);
      Exit;
    end;
    ...
    begin
      Close;
      MessageBox(Application.Handle, pchar('Senha inválida.'), 'Erro', MB_ICONWARNING);
      FechaDataModule(dtmCon);
      Result := LoginDB(u,s);
      Exit;
    end;
