...
begin
  ...
  if cfmHandle = 0 then
  begin
    MessageBox(Application.Handle, pchar('ConsultaCNPJ.dll não pode ser carregada.'), 'ops!', MB_ICONERROR);
    exit;
  end;
  ...
  try
    ...
    on e : Exception do
    begin
      MessageBox(Application.Handle, pChar('Não foi possível executar a função ConsultaCNPJ.'+#13+#13+e.message), 'Erro', MB_ICONERROR);
      exit;
    end;
  end;
