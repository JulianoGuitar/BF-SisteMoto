...
//  MessageBox(Handle, pchar('Em construção.'), 'Aviso', MB_ICONWARNING);
  // AbreTela('SGV307','SGV.BPL',sUsuario,sPath_Local,'', sBancoDeDados,cCorBluefield,cCorNormal,iEmp, 1,true);
    ...
    if MessageBox(Handle, pchar('Impressão ok?'), 'Impressão Pedido', MB_YESNO+MB_ICONQUESTION) = id_yes
    then
    with dtmCon.qrApoio do
    try
      td.TransactionID := Cardinal(FormatDateTime('ddmmyyyy',date)+FormatDateTime('hhnnss',time));
...
begin
  ...
  case t of
    ...
    1 :
    begin
      a := 'Deseja reenviar este item para impressão?';
      if (cdsIMPRESSO.AsString <> 'S')
      then
      begin
        MessageBox(Handle, pchar('Item ainda não foi impresso!'), 'Aviso', MB_ICONWARNING);
        exit;
      end;
    end;
  end;
  ...
  if MessageBox(Handle, pchar(a+#13
                             ...
                             +#13+'Confirma?')
                             ,'Confirmar operação', MB_YESNO+MB_ICONQUESTION+MB_DEFBUTTON2) = id_no
  then exit;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        PrintLeft(FormatDateTime('dd/mm/yy hh:mm:ss', cdsDT_INCLUI.AsDateTime)
                 +' | '+cdsCOMANDA.AsString+' '+cdsCOMANDA_NOME.AsString
                 , MarginLeft);
        NewLine;
        if LinesLeft <= 0 then NewPage;
